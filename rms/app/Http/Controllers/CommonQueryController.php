...
//end search query
    ...
    public static function stateOfMonitoring($container, $search)
    {
        ...
        $tm = ReeferMonitoring::query()->where('container_id', $container->id)
            ->whereRaw('reefer_monitorings.date <= ? ', [Carbon::parse($search->date)->toDateString()])
            ->latest('date');

        $tm = DB::table('reefer_monitorings')->whereRaw('container_id = ? AND date <= ? ',[$container->id,$search->date])->orderBy('date','DESC')->limit(1);
        $monitoring = $tm->first();

        $d = '';
        $prot->prev_monitoring = null;
        $prot->next_monitoring = null;
        if($monitoring!=null){
            $d = $monitoring->date;
...
<div>query:".$tm->toSql()."</div>
<div>query:".implode(',',$tm->getBindings())."</div>
<div>query:".json_encode((array)$monitoring)."</div>
</span>";
